...
# -----------------------------
# -----------------------------
# 1. Updated Amino Acids with PAD token
# -----------------------------
# We add '-' as a padding character. Total classes: 21
...
# -----------------------------
# -----------------------------
# 2. Updated Dataset with Padding Logic
# -----------------------------
class PeptideDataset:
    def __init__(self, peptides, y_f, y_s=None):
        ...
        # Sequence lengths for conditioning
    ...
    def create_targets_with_mask(self):
        N = len(self.peptides)
        # X: (N, max_len, 21)
        X = np.zeros((N, self.max_len, NUM_CLASSES), dtype=np.float32)
        # targets: (N, max_len) - initialized with PAD_IDX
        targets = np.full((N, self.max_len), fill_value=PAD_IDX, dtype=np.int64)
        ...
        for i, p in enumerate(self.peptides):
            ...
            # Fill the remainder of X for this row with the PAD one-hot vector
            if len(p) < self.max_len:
                for j in range(len(p), self.max_len):
                    X[i, j, PAD_IDX] = 1.0
                    # targets[i, j] remains PAD_IDX from initialization
...
def train_cvae(csv_path, epochs=50, batch_size=16, lr=1e-3, latent_dim=32, model_dir="models", device=None):
    ...
    encoder = Encoder(input_dim=NUM_CLASSES, hidden_dim=128, latent_dim=latent_dim, seq_len=seq_len, cond_dim=2).to(device)
    decoder = Decoder(latent_dim=latent_dim, hidden_dim=128, output_dim=NUM_CLASSES, seq_len=seq_len, cond_dim=2).to(device)
    ...
    for epoch in range(epochs):
        ...
        for x_batch, target_batch, cond_batch in data_loader:
            ...
            loss_rec = ce_loss(x_recon_logits.view(-1, NUM_CLASSES), target_batch.view(-1))
...
def main():
    ...
    # -----------------------------
    model_dir = MODEL_DIR + "/baseline_1_corrected_may_2026"
